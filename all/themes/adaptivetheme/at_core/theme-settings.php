...
function adaptivetheme_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  if (at_get_setting('atcore_version_test', $theme_name) === 1) {
    ...
    if (!isset($legacy_info['release']) || $legacy_info['release'] === '7.x-2.x') {
      $version_message = t('<p>The version of your theme (@theme) is not designed to run on <a href="!link_project" target="_blank">Adaptivetheme 7.x.3.x</a>. It will probably run, but your experience will not be optimal. You have three courses of action to choose from:</p>', array('!link_project' => 'https://drupal.org/project/adaptivetheme', '@theme' => $theme_name));
    ...
    if (isset($legacy_info['release']) && $legacy_info['release'] === '7.x-3.x') {
      $version_message = t('<p>This theme (@theme) is compatible with <a href="!link_project" target="_blank">Adaptivetheme 7.x.3.x</a>. You are good to go! You can turn off this message in the Debug settings, look for "Sub-theme compatibility test".</p>', array('!link_project' => 'https://drupal.org/project/adaptivetheme', '@theme' => $theme_name));
